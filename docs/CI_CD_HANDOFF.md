...
### `production` environment 需要的 secrets
...
- 如果希望做到全自动，不要给 `production` 配置人工审批
...
```text
OC_DB_DSN=openchat:<密码>@tcp(host.docker.internal:3306)/openchat?parseTime=true&charset=utf8mb4
```
...
5. 验证：
   - GHCR 包是否在主仓库所属账号下生成
   - test 栈是否健康
6. 配置 `production` environment secrets
7. 准备 `/srv/catscompany-prod/env/prod.env`
8. 确认 prod 能通过 `OC_DB_DSN` 连到现有 MySQL
9. 允许 `Deploy Docker Prod` 运行
10. 验证 prod 影子栈的 `26061` / `28080`
11. 单独决定 nginx 的切流方式
